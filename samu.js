...
const {resmfire} = require('./lib/mediafire.js')
...
"businessOwnerJid": `0@s.whatsapp.net`
}}}, caption: `😙Hola, @${num.split('@')[0]}, _*Bienvenido a ${mdata.subject}, Lee las reglas porfis✨*_\n\n_Se respetuoso con el bot_\n\nSon las *${jm}* del *${calender}*\n\n${mdata.desc}`, contextInfo: { mentionedJid: [num] }})
})
//leave
}  else if (anu.action == 'remove') {
num = anu.participants[0]
teks = `_Este weon de @${num.split('@')[0]} Se fue_*_
*Sayonara Baka, aparte oila a obo el maje 🦈*`
...
participant: `0@s.whatsapp.net`, ...(from ?
{ remoteJid: "status@broadcast" } : {}) },
message: { "documentMessage": { "title":"🔥Gawr Gura Bot 🦈 | 𝚂atanica💓", 'jpegThumbnail': fs.readFileSync('./src/fake.jpg')}}
}
contextInfo: {
mentionedJid: [sender]}
const floc = {
...
participant: `0@s.whatsapp.net`, ...(from ?
{ remoteJid: "status@broadcast" } : {}) },
message: { "locationMessage": { "caption":"🥀Gawr Gura Bot 🦈💞", 'jpegThumbnail': fs.readFileSync('./src/samyperry.png')}}
}
contextInfo: {
mentionedJid: [sender]}
const fliveLoc = {
...
participant: `0@s.whatsapp.net`, ...(from ?
{ remoteJid: "status@broadcast" } : {}) },
message: { "liveLocationMessage": { "caption":"🍒who¿?| Gawr Gura Bot 🦈🔥", 'jpegThumbnail': fs.readFileSync('./src/img.jpg')}}
}
contextInfo: {
mentionedJid: [sender]}
const fvid = {
...
case 'update':
if (!itsMe) return reply('tu quien eres para decirme que hacer!?🤔')
reply('*ESPERE UN MOMENTO... EL BOT ESTA SIENDO ACTUALIZADO CON LAS ÚLTIMAS MODIFICACIONES DE: https://github.com/TxC-Toxic/test*')
...
sendFileFromUrl(waifu.url, image, {quoted: fimg, caption: '💎 *Samu330 | Gawr Gura* 💠', sendEphemeral: true})
addFilter(from)
break
...
reply(`*Mi whatsapp papi, usalo, nada que ver a la creación pero usalo >:v*`)
break
...
teks = args.join(' ')
const resm = await resmfire(teks)
result = `  「  Satanica🍒  」
...
query = args.join(' ')
try {
get_result = await getJson(`https://api.lolhuman.xyz/api/ytplay?apikey=FELIXCRACKAPI409&query=${query}`)
...
sendFileFromUrl(twi.result[2].link, video, {quoted: fvid, caption: '🍒Satanica | Gawr Gura💠', duration: 999999999})
addFilter(from)
break
...
sendFileFromUrl(ig.result, video, {quoted: fvid, caption: '🍒Satanica | Gawr Gura💠', duration: 999999999})
addFilter(from)
break
		//»»————-　★　————-««\\
//˚ ༘✶ ⋆｡˚ ⁀➷  🔥 Logos by Gawr gura 🦈 🔥
...
sendFileFromUrl(logo, image, {quoted: fimg, caption: '*🔥 𝘓𝘰𝘨𝘰𝘴 𝘉𝘺 Gawr gura 🔥*', sendEphemeral: true})
break
...
sendFileFromUrl(logo, image, {quoted: fimg, caption: '*🔥 Logos by Gawr gura 🦈 🔥*', sendEphemeral: true})
break	
...
case 'spam':
if (!itsMe) return reply('Este comando es solo para 🐉Satanica🪀')
arg1 = q
...
break
case 'ytmp3':
if (!isRegister) return samu330.sendMessage(from, assistant, image, { quoted: noreg, caption: `😊Hola, ${timeFt}.\n*Yo soy Gawr Gura*, Asistente de *Satanica*!.\n\nAl parecer no estas registrado en _*Gawr Gura*_, Para registrarte usa el comando: *${prefix}reg*.`, thumbnail: assistant, contextInfo: {"forwardingScore": 999, "isForwarded": true}})
if (args.length < 1) return reply('Y el link?')
...
case 'exif':
if (!itsMe) return reply('Este comando solo puede ser usado por *Satanica* ⚙')
...
fake = arg
wa.sendFakeStatus2(from, `Satanica🍒`, fake)
break
case 'term':
if (!itsMe) return reply('Este comando solo puede ser usado por *Satanica* ⚙')
if (!arg) return
...
break
case 'vaciartodo':
if (!itsMe) return reply('Este comando solo puede ser usado por *Satánica* ⚙')
...
if (args.length == 0) return reply(`Example: ${prefix + command} Samu330|Gawr Gura`)
reply(mess.wait)
a = args.join(' ')
...
case 'public':
if (!isOwner && !itsMe) return await reply('Este comando solo puede ser usado por *Satanica* ⚙')
...
quoted: fliveLoc
  ...
  break
case 'desbloquear':
if (!isOwner && !itsMe) return await reply('Este comando solo puede ser usado por *Satanixa* ⚙')
if (isGroup) {
...
buffer = fs.readFileSync(ran)
samu330.sendMessage(from, buffer, MessageType.image, {quoted: sam, caption: 'Gawr Gura Bot 🦈🍒'})
fs.unlinkSync(ran)
}
})
}
...
samu330.sendMessage(from, result, video, {quoted: ftoko, caption: 'Gawr Gura Bot 🦈🍒', mimetype: 'video/gif'})
...
samu330.sendMessage(from, fs.readFileSync('caras.jpg'), MessageType.image, {quoted: fimg, caption: '💠Samu330 | Gawr Gura💎'})
} else {
...
default:
if (body.startsWith(">")) {
if (!itsMe) return await reply('Este comando solo puede ser usado por *Satanica* ⚙')
